...
pub const HELP: &[HelpEntry] = &[
    ...
    HelpEntry {
        command: "show",
        short: "Open the draft browser, optionally focused on a specific draft",
        purpose: "Same TUI as `pcr bundle`. With a number, opens focused on that draft; without one, opens at the newest. Use it to inspect, multi-select, and bundle drafts.",
        when_to_use: "Whenever you want to look at, organize, or ship drafts. `pcr show` and `pcr bundle` (no args) are aliases — use whichever name reads better.",
        examples: &[
            ("pcr show", "open the draft browser at the newest draft"),
            ("pcr show 3", "open the browser focused on draft #3"),
            ("pcr show --all", "browse every draft, not just the most recent 100"),
            ("pcr show --repo cli", "filter to drafts touching the cli repo"),
            ("pcr --plain show 3", "print draft #3 to stderr instead of opening the TUI"),
        ],
        see_also: &["log", "bundle", "push", "gc"],
        runnable: Runnable::Direct,
    },
    HelpEntry {
        command: "bundle",
        ...
        examples: &[
            ("pcr bundle", "browse the most recent 100 drafts in the TUI (same as `pcr show`)"),
